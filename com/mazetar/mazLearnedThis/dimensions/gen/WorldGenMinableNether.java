...
public class WorldGenMinableNether extends WorldGenerator {
    ...
    public WorldGenMinableNether(int id, int number) {
        // Change this block to the block generated in the End, Nether, Your
        // dimension, my dimension has my custom stone in it so for me
        // i would have TutorialStone
        this(id, number, Block.netherrack.blockID);
    }

    public WorldGenMinableNether(int id, int meta, int number, int target) {
        this(id, number, target);
        minableBlockMeta = meta;
    }

    public WorldGenMinableNether(int id, int number, int target) {
        this.minableBlockId = id;
        this.numberOfBlocks = number;
        this.targetBlock = target;
    }

    public boolean generate(World world, Random random, int par3,
            int par4, int par5) {
        float f = random.nextFloat() * (float) Math.PI;
        double d0 = (double) ((float) (par3 + 8) + MathHelper.sin(f)
                * (float) this.numberOfBlocks / 8.0F);
        ...
        double d3 = (double) ((float) (par5 + 8) - MathHelper.cos(f)
                * (float) this.numberOfBlocks / 8.0F);
        double d4 = (double) (par4 + random.nextInt(3) - 2);
        double d5 = (double) (par4 + random.nextInt(3) - 2);

        for (int l = 0; l <= this.numberOfBlocks; ++l) {
            double d6 = d0 + (d1 - d0) * (double) l
                    / (double) this.numberOfBlocks;
            ...
            double d10 = (double) (MathHelper.sin((float) l * (float) Math.PI
                    / (float) this.numberOfBlocks) + 1.0F)
                    * d9 + 1.0D;
                        ...
                        if (d12 * d12 + d13 * d13 < 1.0D) {
                            for (int i3 = k1; i3 <= j2; ++i3) {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D 
                                        && (block != null && block.isGenMineableReplaceable(
                                                        world, k2, l2, i3,
                                                        targetBlock))) {
                                    world.setBlock(k2, l2, i3,
                                            this.minableBlockId,
                                            minableBlockMeta, 2);
                                }
                            }
